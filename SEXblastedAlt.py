    ...
    for strain in pbar:
        ...
        with open(strain, 'r') as file:
            ...
            for row in file:
                ...
                qstart = int(row.split("\t")[6])
                qend = int(row.split("\t")[7])
                            ...
                            if all([prom_idf == prom_idp, int(align_length) >= prom_length-int(diff)]):
                                ...
                                for fsa in SeqIO.parse(strains_dir + str_only + "/SC1_" + str_only + '.fsa', "fasta"):
                                    ...
                                    if node_idf in repr(fsa.id):
                                        ### check if the extracted sequence begins/ends at the same place as the node
                                        if any([sstart == 1, sstart == len(fsa.seq), send == 1, send == len(fsa.seq)]):
                                            print("promoter:", prom_idf.split("_")[3], " node:", node_idf, "\nstart:", sstart, " end:", send, "\nnode length:", len(fsa.seq), " reference length:", prom_length)
                                        else:
                                           ### and extract intergenic region of this particular strain
                                           prom_out.write(">" + fsa.id + "|" + prom_idp + "|evalue:_" + evalue + "|strand:_")
                                           if sstart < send:
                                               out_length = len(fsa.seq[sstart-1:send]) + 1
                                               if out_length < prom_length:
                                                   sstart -= (qstart - 1)
                                                   send += (prom_length - qend - 1)
                                                   if send > len(fsa.seq):
                                                       send = len(fsa.seq)
                                               prom_out.write("+\n%s\n" % fsa.seq[sstart-1:send])
                                           else:
                                               out_length = len(reverse_complement(fsa.seq[send-1:sstart])) + 1
                                               if out_length < prom_length:
                                                   sstart += (qstart - 1)
                                                   if sstart > len(fsa.seq):
                                                       sstart = len(fsa.seq)
                                                   send -= (prom_length - qend - 1)
